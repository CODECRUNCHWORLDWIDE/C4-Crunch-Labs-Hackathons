...
```mermaid
flowchart TD
  PM["PM - Scope-keeper"] --> Builder["Full-stack builder"]
  PM --> Designer["Designer"]
  PM --> Pitch["Pitch lead"]
  Builder --> Demo["Working demo"]
  Designer --> Demo
  Pitch --> Demo
```
*The PM sits over the other three roles; all three feed into the one thing that gets judged, the demo.*
...
```mermaid
flowchart TD
  Start["Evaluate a sponsor prize"] --> Fit{"Fits your project without distorting it"}
  Fit -->|"No"| Skip["Skip it - pick zero"]
  Fit -->|"Yes"| Count{"Already picked two prizes"}
  Count -->|"Yes"| Skip2["Skip this one too"]
  Count -->|"No"| Pick["Chase this prize"]
```
*Walking the fit-then-cap check before committing to a sponsor prize.*

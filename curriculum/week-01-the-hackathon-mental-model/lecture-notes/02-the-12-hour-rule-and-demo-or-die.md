...
```mermaid
flowchart TD
  Check["Hour 12 - measure remaining work vs remaining time times 0.7"] --> Fits{"Remaining work fits the buffer"}
  Fits -->|"Yes"| Continue["Keep building as planned"]
  Fits -->|"No"| Cut["Cut scope now"]
  Cut --> Redraw["Redraw the MoSCoW chart"]
```
*The hour-12 check: either the remaining work fits the 30 percent buffer, or something gets cut.*
...
```mermaid
flowchart LR
  A["Build frozen"] --> B["Demo rehearsal"]
  B --> C["Demo recording"]
  C --> D["Pitch rehearsal"]
  D --> E["Submission"]
  E --> F["Rest"]
```
*The final three hours as a straight pipeline, no branching, no new features.*

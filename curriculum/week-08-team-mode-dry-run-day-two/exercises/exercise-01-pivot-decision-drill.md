...
The team chose, at hour 2, to target FIU learners timing their walk to
...
At hour 14:00, Sam (Demo Lead) joins the call and proposes:
  ...
  "Hey — what if we changed the target user to *Miami bus commuters*
   instead of FIU learners? The seeded data for bus routes is way more
...
The team built a "queue-of-learners-needing-help" page for a fake CS
tutoring center. MUST list: 1) a tutor-side queue view sorted by
wait-time, 2) a learner-side "request help" form, 3) a real-time
push notification to the tutor when a new learner joins the queue.

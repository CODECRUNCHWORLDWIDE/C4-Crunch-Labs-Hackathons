...
```mermaid
flowchart TD
  A["SHOULD or COULD item"] --> B{"Screen recordable in three minutes"}
  B -- "No" --> C["Cut"]
  B -- "Yes" --> D{"Needs more than thirty minutes polish"}
  D -- "No" --> E["Keep"]
  D -- "Yes" --> F{"Script survives without it"}
  F -- "Yes" --> G["Cut"]
  F -- "No" --> H["Escalate - rebudget or rewrite script"]
```
*The hour-18 demo cut applies the same three-question checklist to every SHOULD or COULD item.*

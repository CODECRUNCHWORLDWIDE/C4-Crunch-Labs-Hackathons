...
```mermaid
sequenceDiagram
  participant OffLead as Rotating-off Lead
  participant OnLead as Rotating-on Lead
  participant Comms as Comms Lead
  OffLead->>OnLead: Announce merge state and next PR
  OnLead->>OffLead: Accept Builder Lead role
  OnLead->>Comms: Confirm hold on new merges until landed
  Comms->>Comms: Log both statements in DAY-2-LOG.md
```
*The hour-12 Builder Lead rotation is announced and accepted aloud, then logged verbatim.*
...
- **A user change.** Changing the target user from "freshmen" to "transfer learners" *is* a pivot, because the user defines the prompt response.
...
```mermaid
flowchart TD
  A["Proposed change surfaces"] --> B{"Q1 - user pain or verb change"}
  B -- "No - implementation only" --> C["Not a pivot - adjust and continue"]
  B -- "Yes" --> D{"Q2 - ships in remaining hours against same prompt"}
  D -- "No" --> E["No pivot - cut a MUST instead"]
  D -- "Yes" --> F["Pivot - log what is kept and discarded"]
```
*Most pivot conversations resolve at Q1 or Q2 without ever changing the build's direction.*
...
"FIU transfer learners" because the seeded data for the transfer-learner
...
script's hook line 2 to reference *both* freshmen and transfer learners
as an aside, without changing the build's user model.

**Alternative considered (and rejected):**
Pivoting to transfer-learner target — rejected on rubric q2.

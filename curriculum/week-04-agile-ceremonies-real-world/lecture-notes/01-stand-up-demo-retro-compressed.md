...
```mermaid
flowchart TD
  A["Ceremony ends"] --> B{"Did it produce a decision"}
  B -->|"Yes"| C["Ceremony succeeded"]
  B -->|"No"| D{"Which ceremony failed"}
  D --> E["Stand up gave status only the silent stand up failure"]
  D --> F["Demo ended in applause the demo as showcase failure"]
  D --> G["Retro named a person not a system the blame retro failure"]
  D --> H["A point got read as hours the points as promises failure"]
```
*A ceremony without a named decision is a meeting; the failure mode tells you which repair to run.*
...
```mermaid
flowchart TD
  A["Hour 0 opening stand up plus planning poker"] --> B["Hour 6 stand up two"]
  B --> C["Hour 12 stand up plus internal demo"]
  C --> D["Hour 18 stand up four"]
  D --> E["Hour 24 stand up plus internal demo"]
  E --> F["Hour 30 stand up six"]
  F --> G["Hour 33 judge demo dress rehearsal"]
  G --> H["Hour 36 submit then team retro"]
```
*The ceremony clock threading five stand-ups and two internal demos across the 36-hour event.*

...
```mermaid
flowchart TD
  A["Facilitator reads the item"] --> B["Teammates think silently"]
  B --> C["Reveal cards simultaneously"]
  C --> D{"Are all cards the same"}
  D -->|"Unanimous"| E["Record the point and move on"]
  D -->|"Split"| F["Highest and lowest argue for twenty seconds each"]
  F --> G["Team re votes once"]
  G --> H{"Still split"}
  H -->|"Yes"| I["Record as a question mark card"]
  H -->|"No"| E
```
*Each backlog item moves through silent thinking and a simultaneous reveal before landing on a size.*
...
```mermaid
flowchart TD
  A["Teammate proposes scope"] --> B["Acknowledge the concern"]
  B --> C["Refuse with the contract reason"]
  C --> D["Offer a cheaper alternative"]
  D --> E{"Does the teammate accept"}
  E -->|"Yes"| F["Build the cheaper version"]
  E -->|"No"| G["Route to the next stand up cut question"]
  G --> H["Require an equal points cut from MUST"]
```
*The three-sentence no script, and where an unresolved disagreement goes next.*

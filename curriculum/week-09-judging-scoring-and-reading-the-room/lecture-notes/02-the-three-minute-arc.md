...
```mermaid
flowchart LR
  A["Problem - 30s"] -->|"scores originality"| B["Demo - 90s"]
  B -->|"scores design and polish"| C["Tech - 45s"]
  C -->|"scores technical complexity"| D["Ask - 15s"]
  D -->|"scores presentation"| E["Judge scores the pitch"]
```
*The four-beat pitch arc and which rubric dimension each beat gives the judge evidence for.*
...
Anti-patterns:
- **Starting with the team's bio.** "We are four CS learners from FIU." The judge does not care; the judge cares about the user.

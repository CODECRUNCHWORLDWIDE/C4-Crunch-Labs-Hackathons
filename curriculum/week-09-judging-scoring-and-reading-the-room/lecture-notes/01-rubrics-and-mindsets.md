...
> **C4 voice on originality:** originality is *narrow target* + *specific feature*. The team that targets "learners" loses to the team that targets "FIU CS learners walking from dorms to morning classes." The narrow target is the originality lever; the specific feature is the proof.
...
- **In the demo beat (90s):** show the build *and* tell a one-sentence user story — "this is Maya, a junior CS learner at FIU, two minutes before her 8am class." The story-frame is the investor-judge's hook.
...
- **In the ask beat (15s):** make the ask more concrete for investor-judges. "We will spend the next two weeks talking to ten more FIU learners and shipping the saved-items feature." A specific next step beats a vague "we hope you like it."
...
```mermaid
flowchart TD
  A["Read judge cues in first 30 seconds"] --> B{"2 or more engineer cues?"}
  B -->|Yes| C["Judge as engineer"]
  B -->|No| D{"2 or more investor cues?"}
  D -->|Yes| E["Judge as investor"]
  D -->|No| F["Mixed or unclear - keep default arc"]
  C --> G["Lean into tech beat - name one decision"]
  E --> H["Lean into problem beat - user story framing"]
  F --> I["Keep 30-90-45-15 balanced"]
```
*How the team reads judge cues within the first 30 seconds and shifts pitch emphasis for engineer versus investor mindsets.*

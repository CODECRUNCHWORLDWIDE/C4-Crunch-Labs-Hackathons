...
```mermaid
flowchart TD
  A["Each teammate writes their own MUST list"] --> B["Comms Lead captures the union list"]
  B --> C["Items every teammate listed become MUST"]
  B --> D["Items one or two teammates listed become SHOULD or COULD"]
  C --> E["Team negotiates the third MUST in five minutes"]
  D --> F["Final three MUST list with owners"]
  E --> F
```
*The union list of everyone's wants gets cut down to the three committed MUSTs.*
...
```mermaid
flowchart TD
  A["Track blocked for 30 minutes"] --> B{"Another MUST ready to work on"}
  B -->|Yes| C["Switch to the next MUST item"]
  B -->|No| D{"A SHOULD or COULD available"}
  D -->|Yes| E["Switch to a SHOULD for at most one hour"]
  D -->|No| F["Escalate to the Builder Lead"]
  F --> G["Five minute unblock huddle"]
  G --> H["Decision: deliver now, merge anyway, or cut to SHOULD"]
```
*The switch-or-escalate protocol a track follows once it has been blocked past 30 minutes.*

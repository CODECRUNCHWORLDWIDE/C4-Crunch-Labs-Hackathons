...
```mermaid
flowchart LR
  A["Intros and intent"] --> B["Prompt reading"]
  B --> C["Role assignment"]
  C --> D["Scope pass one"]
  D --> E["Comms setup"]
  E --> F["Start coding handoff"]
```
*The six 10-minute segments of the kickoff run in fixed order, each with a named output.*
...
```mermaid
flowchart TD
  A["Each teammate scores every role 0 to 3"] --> B["Read the highest score per role column"]
  B --> C{"Tie at 3 for a role"}
  C -->|No| D["Assign the role to the highest scorer"]
  C -->|Yes| E["Give the role to the teammate with the lower max elsewhere"]
  D --> F["Leftover teammate becomes floating builder"]
  E --> F
```
*How role-fit grid scores convert into Builder, Demo, and Comms leads, with ties broken by lower max elsewhere.*

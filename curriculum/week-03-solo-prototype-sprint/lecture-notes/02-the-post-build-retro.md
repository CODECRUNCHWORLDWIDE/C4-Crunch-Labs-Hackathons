...
```mermaid
flowchart LR
  A["Scratch file evidence"] --> B["Keep"]
  A --> C["Start"]
  A --> D["Stop"]
  B --> E["Three behaviors to change"]
  C --> E
  D --> E
```
*Evidence from the build feeds keep, start, and stop, which distill into the three-behaviors line.*
...
```mermaid
flowchart LR
  A["During the build: scratch file notes"] --> B["Hour 5:30 to 6:00: draft in the freeze"]
  B --> C["Sunday morning: polish with sleep distance"]
  C --> D["Commit to retro RETRO.md"]
```
*Two writing passes: hot draft during the freeze, cool polish after a night of sleep.*

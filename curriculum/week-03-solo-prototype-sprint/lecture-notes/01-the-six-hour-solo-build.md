...
- *A vegetarian learner* can paste a recipe and get a meat-free swap suggestion.
...
```mermaid
flowchart LR
  A["Hour 0 Opening Ritual"] --> B["Hour 1 Stand-up"]
  B --> C["Hour 2 Stand-up"]
  C --> D["Hour 3 Stand-up"]
  D --> E["Hour 4 Stand-up"]
  E --> F["Hour 5 Freeze"]
  F --> G["Hour 6 Demo Recording"]
```
*The six-hour clock: open with the ritual, run the hourly cut reflex four times, freeze, then record the demo.*
...
```mermaid
flowchart LR
  A["Monday Lecture 1"] --> B["Tuesday Exercise 1"]
  B --> C["Wednesday Exercise 2"]
  C --> D["Thursday Lecture 2"]
  D --> E["Friday Exercise 3"]
  E --> F["Saturday Mini-Project"]
  F --> G["Sunday Quiz and Retro Polish"]
```
*The Week 3 sequence: doctrine, prep, build, then retro.*

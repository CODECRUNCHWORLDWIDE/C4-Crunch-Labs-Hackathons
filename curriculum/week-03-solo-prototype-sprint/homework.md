...
**Problem statement.** Take this deliberately bad prompt: *"An app to help learners with their lives."* Re-write it into a one-sentence prompt in the `<user> can <verb> to relieve <pain>` shape, with one specific named human in mind (you, a roommate, a classmate). Then write the two-sentence demo-screen description.

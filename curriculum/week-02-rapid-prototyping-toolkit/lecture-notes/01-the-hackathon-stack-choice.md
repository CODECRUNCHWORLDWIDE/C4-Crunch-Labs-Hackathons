...
```mermaid
flowchart LR
  A["Hackathon prompt type"] -->|"Tool or CRUD"| S1["C4 default: React plus FastAPI"]
  A -->|"All TS single deploy"| S2["Next.js full-stack"]
  A -->|"All TS type-safety love"| S3["T3 stack"]
  A -->|"Content or marketing"| S4["Astro plus SQLite"]
  A -->|"ML or AI demo"| S5["Streamlit or Gradio"]
```
*The choice matrix as a decision tree: what the prompt looks like decides the stack, not what sounds impressive.*
...
```mermaid
flowchart LR
  A["Clone template"] --> B["Install deps"]
  B --> C["Frontend dev server runs"]
  C --> D["Backend dev server runs"]
  D --> E["Push to GitHub"]
  E --> F["Trigger Vercel deploy"]
  F --> G["Trigger Railway deploy"]
  G --> H["Visit both URLs - confirm 200"]
```
*The eight steps that must complete inside the 30-minute scaffold-to-first-route window.*

...
```mermaid
flowchart LR
  A["Hour 2: quick add proposed"] --> B["Cut: small item to SHOULD else WONT"]
  B --> C["Hour 12: team tempted to promote a card"]
  C --> D["Cut: promote only if clickable on live URL"]
  D --> E["Hour 24: feels-missing panic"]
  E --> F["Cut: README paragraph, no new build"]
```
*The three predictable cut moments across the event clock, each trigger paired with its pre-written response.*
...
```mermaid
flowchart TD
  W["Scoping worksheet"] --> WONT["WONT list"]
  W --> CUTLIST["Cut-list"]
  W --> MUSTCOL["MUST column"]
  WONT --> NOSCRIPT["Contract no script"]
  CUTLIST --> STANDUP["Stand-up Q4: what do we cut"]
  MUSTCOL --> DEMO["Internal demo at hour 12 and 24"]
```
*The worksheet's three threads each feed a specific piece of the Week 4 team contract.*

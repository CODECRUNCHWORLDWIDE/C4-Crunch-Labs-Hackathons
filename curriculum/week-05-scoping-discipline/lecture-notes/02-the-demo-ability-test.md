...
```mermaid
flowchart LR
  Q1["URL reachable now"] --> Q2["Stranger opens with no setup"]
  Q2 --> Q3["Stranger performs verb in 60 sec"]
  Q3 --> Q4["Verb works on live URL not localhost"]
  Q4 --> Q5["Seeded data visible"]
  Q5 --> PASS["Five yeses: demo-able"]
  Q1 -.->|any no| GAP["Name the gap"]
  Q2 -.->|any no| GAP
  Q3 -.->|any no| GAP
  Q4 -.->|any no| GAP
  Q5 -.->|any no| GAP
```
*Each MUST item runs this five-question gauntlet; one no anywhere routes to a named gap instead of a pass.*
...
```mermaid
flowchart TD
  A["All MUST items pass five of five"] -->|Yes| B["Keep MUST as is"]
  A -->|No| C["Cut decision required"]
  C --> D["Cut failing MUST to SHOULD or WONT"]
  C --> E["Pivot to a simpler version"]
  C --> F["Redeploy a fix, hard retest at hour 16"]
```
*The hour-12 branch: a clean pass keeps MUST untouched, a failure forces one of three cut options, defaulting to cut over extension.*

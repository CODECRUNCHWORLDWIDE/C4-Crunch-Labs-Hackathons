...
```mermaid
flowchart LR
  A["Hook 0 to 30 sec - name the user"] --> B["Problem 30 to 75 sec - narrow the pain"]
  B --> C["Solution 75 to 165 sec - three clicks on live URL"]
  C --> D["Ask 165 to 180 sec - one number one channel"]
```
*The four beats of the three-minute demo, bounded by four timing checkpoints.*
...
- **The problem beat surfaces the cost of the pain.** Not the abstract "learners struggle with X" — the concrete "the average freshman has 4 unanswered late-night questions per week, and 60% of them give up before getting an answer." The number is the cost.
...
- **Generic pain.** "Learners struggle." Test: which learner? When? What does the struggle cost? If you cannot answer in one sentence each, the pain is generic.
...
- **Solution-flavored problem.** "Learners do not have a peer-help platform." Test: this is a missing-feature statement, not a pain. Re-state the pain in terms of *what the user experiences*, not what the market lacks.
...
```mermaid
sequenceDiagram
  participant P as Pitcher
  participant U as Live URL
  participant A as Audience
  P->>U: Click 1 - sign up and post question
  U-->>A: Signup and post appear live
  P->>U: Click 2 - peer taps reply
  U-->>A: Reply appears on screen
  P->>U: Click 3 - result plus demo clip line
  U-->>A: Result lands - pause two seconds
```
*The three clicks of the solution beat, each producing a visible result for the audience.*
